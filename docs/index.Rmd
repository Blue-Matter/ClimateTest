...
tab = CT_tabulate(Blist, RT=0.7, subset = c(1,2,3,5))
tab = cbind(Management_procedure = MPdesc,tab )

makeCTtab2 = function(tab){
...
<span style="color: #7690ba;">Title: 'Creating and demonstrating a practical, science-based framework that enables fisheries managers to evaluate and adopt climate-resilient management procedures'</span>
...
dat<-data.frame(c("Term","Funded by","Coordinated by","Initiative","Project Partners","TOF Project Leader","Blue Matter Team","UCT Collaborators"),
                 ...
                 c("Sept 2025 - Nov 2027",
                   ...
                   "Kelly Ortega-Cisneros, Carryn de Moor"))
...
<img src="img/schematic.jpg" width = 550>
...
Rather than leaving the investigation of climate resilience stalled in the (perhaps indefinite) wait for scientifically defensible forecasts of climate impacts, the Climate Test presents an alternative approach. The solution proposed here is to shift the focus from forecast-based tests of climate robustness in favor of performance metrics of climate robustness. This is analogous to testing pilots in a simulator under an increasingly difficult set of weather conditions. We do not need a weather forecast to know those pilots that are better at flying in more extreme weather conditions. The Climate Test increases the degree of climate impact to identify those management options that are most robust. 
...
```{r ExpTypes, eval=T}
dat<-data.frame(c("Somatic Growth","Recruitment Strength","Natural Mortality","Condition Factor","Spatial distribution"),
                 ...
                 c("Declines in somatic growth may lead to fewer mature fecund individuals in the population, a higher fraction of fish caught per catch weight, and a lower rate of sustainable harvest",
                   ...
                   "Spatial range contraction towards fishing can increase catchability (fishing efficiency) leading to higher catch per effort and hyperstability in relative abundance indices")
                  )
...
These marginal climate tests can be used to rate MPs according to the level of climate impact before the robustness threshold is crossed. Table 4 shows four marginal tests for six example management procedures for Atlantic Blue Shark (Carruthers 2024b). 
...
(1) Prepare operating models to have no climate change impacts
(2) Tune candidate management procedures so they have identical outcomes given no climate impacts
(3) Test the MPs over increasingly stringent climate impacts
(4) Summarize results

These steps are connected:

<img src="img/steps.png" width = 800>
...
Currently the ClimateTest package is compatible with R version `r R.Version()$version.string`. 
...
```{r Installation_check, eval=T,echo=T, fig.width = 6, fig.height=3.3}
library(ClimateTest)
...
## Demonstration Using OM annd MPs of the ClimateTest Package
...
### Examine Demonstration Operating Models and Management Procedures
...
### Step 1, Prepare Operating Models
...
```{r Step_1_prep_Exp, eval=F,echo=T}
...
```{r Step_2_tune_Exp, eval=F,echo=T}
...
<img src="img/Tuning_screenshot.png" width = 600>
...
<img src="img/Tuning_screenshot2.png" width = 600>



### Step 3, Conduct Climate Tests

The tuned MPs are now projected for incrementally increasing climate tests.  

```{r Step_3_test_Exp, eval=F,echo=T}
...
<img src="img/Test_screenshot.png" width = 600>
...
```{r Step_4_sum_Exp, eval=T,echo=T,fig.width = 6, fig.height=8}
# Raw results
...
## Exploratory Test For N. Atl. Swordfish
