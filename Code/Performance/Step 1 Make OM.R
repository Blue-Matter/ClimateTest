...
source('Code/Performance/Source/Update_growth_pars.R')

OM0 = readRDS("OMs/Robustness_testing/OM_BET.rds")
OM = SubCpars(OM0,1:36)
...
Dep_adjust = Hist0@Ref$ReferencePoints$SSBMSY_SSB0[1]/OM@D[1]  # make starting depletion closer to mean = BMSY (SSB)
...
OM = updateGrowth(OM,t0=0.8,Linf=185) # a little K optimizer to match up with cpars$len_age[1,,nyears]
OM = get_ab(OM)
